...
# nginx配置文件下载地址
url="https://raw.githubusercontent.com/aquasofts/fmgen/main/%E8%84%9A%E6%9C%AC/fm"

# 目标文件路径（包括文件名）
destination="/etc/nginx/sites-available/fm"
...
# 下载文件到临时目录
TEMP_DIR=$(mktemp -d)
ZIP_URL="https://raw.githubusercontent.com/aquasofts/fmgen/main/index/fm123.zip"
ZIP_FILE="$TEMP_DIR/fm123.zip"

# 下载 ZIP 文件
curl -o "$ZIP_FILE" "$ZIP_URL"

# 解压文件到指定目录
DEST_DIR="/etc/nginx/sites-available/fm"
mkdir -p "$DEST_DIR"
...
# 替换/etc/nginx/sites-available/fm中的yourdomain为用户输入的域名
...
# 替换/etc/nginx/sites-available/fm中的jjkk为证书公钥目录
sudo sed -i "s|jjkk|$domain|g" /etc/nginx/sites-available/fm
if [ $? -ne 0 ]; then
    echo "证书公钥替换失败，请检查文件路径和权限。" >&2
    exit 1
fi

# 替换/etc/nginx/sites-available/fm中的hhjj为证书公钥目录
sudo sed -i "s|hhjj|$domain|g" /etc/nginx/sites-available/fm
if [ $? -ne 0 ]; then
...
# 链接配置
if [ -f /etc/nginx/sites-available/fm ]; then
    ...
    echo "/etc/nginx/sites-available/fm 文件不存在。" >&2
    exit 1
fi
